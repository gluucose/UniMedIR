...
def compute_measure(x, pred, data_range):
    ...
    pred_nrmse = compute_NRMSE(x, pred)
    # pred_rmse = compute_RMSE(x, pred)
    return pred_psnr, pred_ssim, pred_nrmse
...
def compute_NRMSE(img1, img2):
    ...
    if type(img2) == torch.Tensor:
        norm = (img2.max() - img2.min()).item()
    else:
        norm = float(img2.max() - img2.min())

    return rmse / norm if norm != 0 else float('inf')  # 避免除以零
